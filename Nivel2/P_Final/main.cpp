...
int main(int argc, char *argv[])
{
    ...
    // Crear una escena para manejar elementos graficos
    ...
    // Cargar la imagen del cronometro
    ...
    // Agregar la particula o jugador
    Particle* particle = new Particle(100, 100);
    scene.addItem(particle);

    // Crear una ave y agregarla a la escena
    Aves* ave = new Aves(200, 200, 1); // Movimiento parabólico
    scene.addItem(ave);

    // Crear la segunda ave y agregarla a la escena
    Aves* ave2 = new Aves(400, 200, 2); // Movimiento lateral
    scene.addItem(ave2);

    // Configurar el temporizador para actualizar la posición de las aves
    QTimer* timer = new QTimer();
    QObject::connect(timer, &QTimer::timeout, [ave, ave2]() {
        ave->updatePosition(); // Actualizar la posición de la primera ave
        ave2->updatePosition(); // Actualizar la posición de la segunda ave
    });
    ...
    textItem->setPos(1160, 25); // Posicion del texto de tiempo restante
    scene.addItem(textItem); // Agregar el texto a la escena


    // Texto para los aciertos
    QGraphicsTextItem* textItemAciertos = new QGraphicsTextItem(QString("0 /14"));
    textItemAciertos->setDefaultTextColor(Qt::green);
    textItemAciertos->setFont(QFont("Arial", 37));
    textItemAciertos->setPos(1124, 620); // Posicion del segundo texto
    ...
    gameOverItem->setPos(250, 120); // Posicion de la imagen
    ...
    QObject::connect(contadorTimer, &QTimer::timeout, [&contador, textItem,textItemAciertos, gameOverItem, youWinPixmap, gameOverPixmap, &scene]() {
        if (contador > 0) {
            ...
            if (contador == 0) {
                ...
                int aciertos = textItemAciertos->toPlainText().split(" ")[0].toInt();
                if (aciertos >= 14) {
